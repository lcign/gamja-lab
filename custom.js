...
(function () {
	...
	var DEF = {
		...
		'--bl-active-bg': '#ff9300', '--bl-active-color': '#212121',
		'--activity-color': '#ff9300', '--highlight-color': '#000000', '--highlight-bg': '#8df900',
		'--pin-color': PIN_DEFAULT, '--buffer-button-background-hover': '#131618',
		'--sidebar-background': '#131618', '--green': '#53b266',
		'--main-background': '#1f2020', '--main-color': '#f8f9fa',
		'--link-color': '#53b266', '--timestamp-color': '#979797',
		'--gray': '#979797', '--red': '#fb615b',
		'--topic-color': '#f8f9fa', '--topic-background': '#1f2020',
		'--button-background': '#008e00', '--button-background-hover': '#008f51',
		'--button-color': '#feffff', '--button-border': '#131618',
				...
				if (!el.hasAttribute('data-ws')) {
					...
					for (k = 0; k < kids.length; k++) {
						...
						if (nd.nodeType === 3) {
							...
							// on an ACTION the leading `* ` goes too, otherwise it is left dangling in front
							// of the text and pushes it out of the column
							if (!tv || (isAction && tv === '*')) { kept.push([k, nd.nodeValue]); nd.nodeValue = ''; }
							else break;
						}
					}
